...
class WalkStore: NSObject {
    ...
    lazy var allWalks: [Walk] = {
        ...
        if let fetchResults = (try? self.managedObjectContext!.executeFetchRequest(fetchRequest)) as? [Walk] {
            return fetchResults
        }
        return [Walk]()
    }()
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("walktracker", withExtension: "momd")!
    ...
    lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator? = {
        ...
        let url = self.applicationDocumentsDirectory.URLByAppendingPathComponent("walktracker.sqlite")
